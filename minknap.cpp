...
#include <iostream>
#include <random> // Para gerar valores aleatórios
...
void *palloc(long size)
{
  ...
  if (size == 0) size = 1;
  if (size != static_cast<size_t>(size)) errorx("Alloc too big %ld", size);
...
void initfirst(allinfo *a, stype ps, stype ws)
{
  ...
  a->d.size  = 1;
  a->d.set1  = static_cast<state*>(palloc(MAXSTATES * sizeof(state)));
...
stype minknap(int n, int *p, int *w, int *x, int c)
{
  ...
  // Corrigindo a atribuição para inttab
  inttab  = static_cast<interval*>(palloc(sizeof(interval) * SORTSTACK));
